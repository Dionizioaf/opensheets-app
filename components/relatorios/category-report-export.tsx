...
import { RiDownloadLine, RiFileExcelLine, RiFilePdfLine, RiFileTextLine } from "@remixicon/react";
...
export function CategoryReportExport({ data, filters }: CategoryReportExportProps) {
  ...
  const exportToCSV = () => {
    try {
      ...
      // Build CSV content
      const headers = ["Categoria", ...data.periods.map(formatPeriodLabel), "Total"];
      const rows: string[][] = [];
  ...
  const exportToPDF = () => {
    try {
      ...
      doc.setFontSize(10);
      doc.text(
        `Período: ${formatPeriodLabel(filters.startPeriod)} - ${formatPeriodLabel(filters.endPeriod)}`,
        14,
        22
      );
      ...
      autoTable(doc, {
        ...
        didParseCell: (data) => {
          ...
          if (data.section === "body" && data.row.index < body.length - 1) {
            const categoryIndex = data.row.index;
            const category = data.categories[categoryIndex];

            if (category && data.column.index > 0) {
